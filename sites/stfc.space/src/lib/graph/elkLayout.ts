 ...
 */
export type ElkCanvasLayoutOptions = { 'elk.direction'?: CanvasDirection } & {
  [key: string]: string;
};
...
function mapNode(nodes: NodeData[], edges: EdgeData[], node: NodeData) {
  const { text, width, height, labelHeight, labelWidth, nodePadding, originalText } =
    formatText(node);
...
export const elkLayout = (
  nodes: NodeData[],
  edges: EdgeData[],
  options: ElkCanvasLayoutOptions = defaultLayoutOptions
) => {
  return new PCancelable<ElkNode>((resolve, reject) => {
    fetch('https://raspy-bush-ae37.stfc.workers.dev', {
      method: 'POST',
      body: JSON.stringify({ ...mapInput(nodes, edges), options })
    })
