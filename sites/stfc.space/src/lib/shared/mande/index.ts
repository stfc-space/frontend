...
export interface MandeInstance {
  ...
  post<T = unknown>(data?: any, options?: Options, fetchOverride?: Window['fetch']): Promise<T>;
  post(
    url: string | number,
    data?: any,
    options?: Options<'text'>,
  ...
  post(data?: any, options?: Options<'text'>, fetchOverride?: Window['fetch']): Promise<string>;
  post(
    url: string | number,
    data?: any,
    options?: Options<'response'>,
 ...
 */
export const defaults: Options & Pick<Required<Options>, 'headers' | 'responseAs'> = {
  responseAs: 'json',
  headers: {
    Accept: 'application/json',
    'Content-Type': 'application/json'
...
export function mande(
  ...
  async function _fetch(
    ...
    return (fetchOverride ? fetchOverride : localFetch)(url, mergedOptions)
      .then((response) =>
        Promise.all([
          response,
      ...
      .then(([response, data]) => {
        ...
        const err = new Error(response.statusText) as MandeError;
  ...
  return {
    ...
    get: (url: string, options?: Options, fetchOverride?: Window['fetch']) =>
      _fetch('GET', url, null, options, fetchOverride),
    delete: (url: string, options?: Options) => _fetch('DELETE', url, null, options)
  };
}

type InferArgs<F> = F extends (api: MandeInstance, ...args: infer A) => any ? A : never;
...
export function nuxtWrap<M extends MandeInstance, F extends (api: M, ...args: any[]) => any>(
  ...
  const wrappedCall: (...args: InferArgs<F>) => ReturnType<F> = function _wrappedCall(
    ...args: InferArgs<F>
  ) {
