<script>
  ...
  import { hoveringKey, hoveringValue } from './linkedChartStores';
  ...
  $: if (valuePosition == 'floating')
    valuePositionOffset =
      (parseInt(gap) + barWidth) * Object.keys(data).indexOf($hoveringKey[linkedKey]) +
      alignmentOffset;
  $: if (type == 'line') polyline = getPolyLinePoints(data);
  $: if (dispatchEvents)
    dispatch('value-update', { value: $hoveringValue[uid], uid, linkedKey, valueElement });
  $: {
  ...
  function getHeight(value) {
    return (
      Math.round((parseInt(height) / highestValue) * value - (type == 'line' ? barWidth / 2 : 0)) ||
      0
    );
  }
  ...
  async function endHover() {
    ...
    if (dispatchEvents)
      dispatch('blur', { uid, linkedKey, valueElement, eventElement: event.target });
  }
</script>
      ...
      {#each Object.entries(data) as [key, value], i}
        {#if type != 'line' || hover}
          <rect
            ...
            opacity={hover && $hoveringKey[linkedKey] && $hoveringKey[linkedKey] != key
              ? fadeOpacity
              : 1}
        ...
        {#if type == 'line'}
          <circle
            fill={hover && $hoveringKey[linkedKey] !== null && $hoveringKey[linkedKey] == key
              ? fill
              : 'transparent'}
            r={barWidth / 2}
...
{#if showValue && ($hoveringValue[uid] || valueDefault)}
  <div
    class="tiny-linked-charts-value"
    style={valuePosition == 'floating'
      ? `position: absolute; transform: translateX(${valuePositionOffset}px)`
      : null}
  >
